...
def generate_indexes_inductive(relations, ids_list, samples_in_training, samples_in_valid, verbose=True):
    ...
    for i in range(len(relations)):
        try:
            ...
            elif match1 in range(samples_in_training, samples_in_training + samples_in_valid) and match2 in range(samples_in_training, samples_in_training + samples_in_valid):
                s1_valid.append([match1 - samples_in_training])
                s2_valid.append([match2 - samples_in_training])
            elif match1 >= (samples_in_training + samples_in_valid) and match2 >= (samples_in_training + samples_in_valid):
                # Both in test set
...
def get_train_and_valid_lengths(params, features, percentage_of_training):
    """
    ...
    number_of_samples_training = int(round(t_all * (1. - VALIDATION_PERCENTAGE)))
    ...
    samples_per_class = int(round(number_of_samples_training / params.NUMBER_OF_CLASSES))

    number_of_samples_training = samples_per_class * params.NUMBER_OF_CLASSES
...
def generate_dataset(params, percentage_of_training, verbose=True):
    """
    ...
    path = Path(__file__).parent.parent / str(params.DATASET + '_reproduce')
    dataset = pd.read_csv(path / str(params.DATASET + '.content'), delimiter='\t', header=None)
    ...
    if params.DATASET != 'CiteSeer':
        y = pd.read_pickle(path / str(params.DATASET + '_y'))

    number_of_samples_training, number_of_samples_validation = get_train_and_valid_lengths(params, x, percentage_of_training)

    p = permute(y.iloc[:,0], int(number_of_samples_training // params.NUMBER_OF_CLASSES))
    ...
    classes = [c[5:] for c in list(y.columns)] # todo maybe adapt here
    ...
    relations = pd.read_csv(path / str(params.DATASET + '.cites'), delimiter='\t', header=None).to_numpy().astype(str)
    ...
    s1tr, s2tr, s1val, s2val, s1te, s2te = generate_indexes_inductive(relations, ids, number_of_samples_training, samples_in_valid=number_of_samples_validation, verbose=verbose)
    ...
    np.save(path / 'relations_transductive', b)
    np.save(path / 'index_x_transductive', s1)
    np.save(path / 'index_y_transductive', s2)
    np.save(path / 'index_x_inductive_training', s1tr)
    np.save(path / 'index_y_inductive_training', s2tr)
    np.save(path / 'relations_inductive_training', btr)
    np.save(path / 'index_x_inductive_validation', s1val)
    np.save(path / 'index_y_inductive_validation', s2val)
    np.save(path / 'relations_inductive_validation', bval)
    np.save(path / 'index_x_inductive_test', s1te)
    np.save(path / 'index_y_inductive_test', s2te)
    np.save(path / 'relations_inductive_test', bte)
