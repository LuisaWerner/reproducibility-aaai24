...
class RelationalKenn(torch.nn.Module):
    ...
    def __init__(self, unary_predicates: [str],
        ...
        if len(self.unary_clauses) != 0:
            self.unary_ke = KnowledgeEnhancer(
                unary_predicates, self.unary_clauses, initial_clause_weight=initial_clause_weight, min_weight=min_weight, max_weight=max_weight,
                boost_function=boost_function)
        if len(self.binary_clauses) != 0:
            self.binary_ke = KnowledgeEnhancer(
                binary_predicates, self.binary_clauses, initial_clause_weight=initial_clause_weight, min_weight=min_weight, max_weight=max_weight,
                boost_function=boost_function)
    ...
    def forward(self, unary: torch.Tensor, binary: torch.Tensor, edge_index: torch.Tensor) \
        ...
        if len(self.binary_clauses) != 0 and len(binary) != 0:
            ...
            deltas_sum, _ = self.binary_ke(joined_matrix)
