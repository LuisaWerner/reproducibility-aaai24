...
class BoostFunction(torch.nn.Module, abc.ABC):
    ...
    def __init__(self, initial_weight: float, fixed_weight: bool,
                 min_weight=0.0, max_weight=500.0):
        super().__init__()
...
class ProductBoostConorm(BoostFunction):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor):
        ...
        truth_max, i = clause_matrix.max(
            dim=1)  # m: maximum literals' truth values, i: the corresponding vector of indexes
        truth = torch.sigmoid(clause_matrix)  # Activations of the literals' truth values
        new_truth_max = torch.sigmoid(
            truth_max + self.clause_weight)  # Activations of the highest literals after adding the delta
        delta_max = new_truth_max - truth[
            np.arange(truth.shape[0]), i]  # Deltas on the activations of the highest literal
